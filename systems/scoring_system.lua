...
function ScoringSystem:createScoreText(x, y, points, distanceFromCenter)
    ...
    if points > 0 then
        ...
        print(
            string.format(
                "HIT! %d points (%.1f pixels from center) - Total: %d points",
                points,
                distanceFromCenter,
                self.world.gameState.totalScore
            )
        )
    else
...
function ScoringSystem:getStats()
    return {
        ...
        averageScore = self.world.gameState.hitCount > 0
                and (self.world.gameState.totalScore / self.world.gameState.hitCount)
            or 0,
    }
end
