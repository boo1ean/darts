...
-- Target Generation System
local TargetGenerationSystem =
    System.new("TargetGenerationSystem", { "Movement", "CircularMovement", "LinearMovement", "CosineMovement" })
...
function TargetGenerationSystem:generateNewTargets(entity)
    ...
    print(
        "Entity",
        entity.id,
        "- AGGRESSIVE FAST targets! Duration:",
        string.format("%.1f", movement.moveDuration),
        "s - EXTREME X-Path Factor:",
        string.format("%.1f", cosine.factor)
    )
