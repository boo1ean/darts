...
local CombinedMovementSystem = System.new(
    "CombinedMovementSystem",
    { "Transform", "CircularMovement", "LinearMovement", "CosineMovement", "Movement" }
)
...
function CombinedMovementSystem:updateCombinedMovement(entity, dt)
    ...
    local endX, endY
    local currentTargetX = linear.previousTargetX
        + (linear.targetX - linear.previousTargetX) * movement.transitionProgress
    local currentTargetY = linear.previousTargetY
        + (linear.targetY - linear.previousTargetY) * movement.transitionProgress

    local currentCenterX = circular.previousCenterX
        + (circular.centerX - circular.previousCenterX) * movement.transitionProgress
    local currentCenterY = circular.previousCenterY
        + (circular.centerY - circular.previousCenterY) * movement.transitionProgress
    local currentAngle = circular.previousAngle
        + (circular.angle - circular.previousAngle) * movement.transitionProgress
    ...
    local currentAmplitudeX = cosine.previousAmplitudeX
        + (cosine.amplitudeX - cosine.previousAmplitudeX) * movement.transitionProgress
    local currentAmplitudeY = cosine.previousAmplitudeY
        + (cosine.amplitudeY - cosine.previousAmplitudeY) * movement.transitionProgress
    local currentFrequencyX = cosine.previousFrequencyX
        + (cosine.frequencyX - cosine.previousFrequencyX) * movement.transitionProgress
    local currentFrequencyY = cosine.previousFrequencyY
        + (cosine.frequencyY - cosine.previousFrequencyY) * movement.transitionProgress
